...
$Script:JsonPath    = Join-Path $BaseDir ("PC-Audit-Report_{0:yyyy-MM-dd_HH-mm}.json" -f $StartTime)
...
$Script:ModuleStatus = [System.Collections.Generic.List[object]]::new()

# Known-good vendor signers whose binaries we won't flag as heavily even if
# other heuristics (path, recency) would otherwise raise the severity.
$Script:TrustedSigners = @(
    ...
    'Realtek Semiconductor Corp.','ASUSTeK Computer Inc.','Dell Inc.','Dell Technologies',
    'Apple Inc.'
)

# Scheduled tasks YOU created and know are legitimate (matched on TaskName,
# case-insensitive, not the full path). A hidden/elevated/user-path
# PowerShell task is exactly what the scanner is supposed to catch, so this
# doesn't silence the finding - it caps its severity to LOW and tags it, so
# your own tooling (e.g. a standby-memory-purge task) doesn't tank the score
# every run. Add your own task names here.
$Script:TrustedTasks = @(
    'clearcache'
)
...
function Add-Finding {
    ...
    # SecurityImpact drives the score; Severity still drives color/labeling.
    # Bloat and Customize are hygiene/preference categories, not risk - a pile
    # of "remove Widgets button" LOW findings should never move the needle,
    # so they're excluded here by Category rather than by Severity or Type
    # (several genuine risk findings, e.g. Defender/firewall being off, are
    # Type='Info' and must still count).
    $impact = if ($Category -in 'Bloat','Customize') { 0 } else {
        switch ($Severity) { 'HIGH' {-20} 'MEDIUM' {-5} 'LOW' {-1} default {0} }
    }
    $Findings.Add([pscustomobject]@{
        Id = $Findings.Count + 1; Category = $Category; Severity = $Severity
        Description = $Description; Type = $Type; Data = $Data; SecurityImpact = $impact
    })
...
# ==============================================================================
# SUPPORT: PROCESS LINEAGE, FILE TRUST, SCORING, JSON EXPORT, ORCHESTRATOR
# ==============================================================================

function Get-ProcessChain {
    <# Walks parent-PID links up to the root process. Returns the chain as an
       array ordered [target, parent, grandparent, ... , root].
       NOTE: named -ProcessId, not -PID - $PID is a reserved PowerShell
       automatic variable holding the *current* session's process ID, and
       shadowing it as a parameter name causes confusing bugs. #>
    param([int]$ProcessId)
...
function Get-FileTrust {
    <# Returns hash + signature info for a file, or $null if it can't be read.
       SignerSummary is the plain certificate subject name (e.g. "Microsoft
       Corporation") so it can be checked against $Script:TrustedSigners. #>
    param([string]$Path)
    ...
    try {
        ...
        if ($sig -and $sig.SignerCertificate) {
            # Pull just the CN= (or O=) value out of the certificate subject
            $subject = $sig.SignerCertificate.Subject
            if ($subject -match 'CN=([^,]+)') { $signer = $Matches[1] }
        }
...
function Get-SecurityScore {
    <# Starts at 100 and sums each finding's SecurityImpact (set in
       Add-Finding). Bloat/Customize findings carry 0 impact, so a clean
       machine buried in "remove Widgets button" suggestions still scores
       near 100 - only Malware/Remote Access/Task Scheduler/Drive/Corruption/
       Files findings actually move the number. #>
    $score = 100
    foreach ($f in $Findings) { $score += $f.SecurityImpact }
...
function Show-FinalScore {
    Write-Section "SECURITY RATING"
    $score = Get-SecurityScore

    # Severity breakdown for findings that actually count toward the score
    $scored = $Findings | Where-Object { $_.SecurityImpact -ne 0 }
    $high   = @($scored | Where-Object Severity -eq 'HIGH').Count
    $medium = @($scored | Where-Object Severity -eq 'MEDIUM').Count
    $low    = @($scored | Where-Object Severity -eq 'LOW').Count
    $hygiene = @($Findings | Where-Object Category -in 'Bloat','Customize').Count

    $color = if ($score -ge 90) {'Green'} elseif ($score -ge 70) {'Yellow'} else {'Red'}
    Write-Line ("  Score: {0}/100" -f $score) $color
    Write-Line ""
    Write-Line ("  HIGH     {0}" -f $high)
    Write-Line ("  MEDIUM   {0}" -f $medium)
    Write-Line ("  LOW      {0}" -f $low)
    Write-Line ("  BLOAT/CUSTOMIZE (0 impact)  {0}" -f $hygiene)
}
...
function Export-JSONReport {
    ...
    try {
        ...
        Write-Line ("  JSON report saved to: {0}" -f $JsonPath) White
    } catch {
        Write-Line ("  JSON export failed: {0}" -f $_.Exception.Message) Yellow
    }
}

function Export-Reports {
    $Report.ToString() | Out-File -FilePath $ReportPath -Encoding UTF8
    Write-Line ("  Full text report saved to: {0}" -f $ReportPath) White
    Export-JSONReport
}

function Invoke-PCSecurityAudit {
    <# Orchestrator: runs each collection module in isolation so one module's
       failure can't take down the rest of the sweep. Interactive review runs
       separately afterward, since it needs to prompt the user in order. #>
    Write-Section "PC SECURITY AUDIT ENGINE v3"
    Write-Line "  Computer : $env:COMPUTERNAME"
    Write-Line "  User     : $env:USERDOMAIN\$env:USERNAME"
    Write-Line "  Started  : $(Get-Date)"

    $modules = @(
        @{ Name = 'Hardware';      Run = { Show-Hardware } }
        @{ Name = 'Storage';       Run = { Show-DriveHealth } }
        @{ Name = 'Defender';      Run = { Invoke-MalwareCheck } }
        @{ Name = 'Integrity';     Run = { Invoke-IntegrityChecks } }
        @{ Name = 'Remote Access'; Run = { Invoke-RemoteAccessAudit } }
        @{ Name = 'Files';         Run = { Invoke-SuspiciousFileSweep } }
        @{ Name = 'Tasks';         Run = { Invoke-TaskSchedulerDeepScan } }
        @{ Name = 'Drivers';       Run = { Invoke-DriverAudit } }
        @{ Name = 'Bloat';         Run = { Invoke-BloatAudit } }
    )

    foreach ($m in $modules) {
        Write-Host ""
        Write-Host "[+] Running $($m.Name)" -ForegroundColor Cyan
        try {
            & $m.Run
            $ModuleStatus.Add([pscustomobject]@{ Module = $m.Name; Status = 'OK'; Error = $null })
            Write-Host "[OK] $($m.Name)" -ForegroundColor Green
        } catch {
            $ModuleStatus.Add([pscustomobject]@{ Module = $m.Name; Status = 'FAIL'; Error = $_.Exception.Message })
            Write-Host "[FAIL] $($m.Name): $_" -ForegroundColor Red
            [void]$Report.AppendLine("[FAIL] $($m.Name): $_")
        }
    }
...
function Invoke-SuspiciousFileSweep {
    ...
    foreach ($f in $suspects) {
        ...
        if ($reasons.Count -ge 2 -or ($reasons -contains 'double extension')) {
            ...
            $trust = Get-FileTrust -Path $f.FullName
            $desc = "{0}  [{1}]  ({2:g}, {3} KB)" -f $f.FullName, ($reasons -join '; '), $f.LastWriteTime, [math]::Round($f.Length/1KB)
            if ($trust) { $desc += ("  | SHA256: {0}  | Signer: {1}" -f $trust.SHA256.Substring(0,16), $trust.SignerSummary) }
            Add-Finding -Category 'Suspicious File' -Severity $sev -Type 'File' -Data $f.FullName -Description $desc
        }
    }
...
function Invoke-TaskSchedulerDeepScan {
    ...
    foreach ($t in $allTasks) {
        ...
        if ($reasons.Count -gt 0) {
            ...
            # Pull trust info (hash + signer) for the resolved binary, and dial
            # severity down a notch if it's signed by a known-good vendor.
            # A recognizable signer name is NOT enough on its own - a valid
            # cert on a copy of google.exe sitting in C:\Users\Public is still
            # suspicious. Require ALL THREE: known signer, a validly-chained
            # signature (not just "any status"), and a path outside the
            # user-writable locations that made this task suspicious in the
            # first place.
            $trust = $null
            if ($resolvedExePath) { $trust = Get-FileTrust -Path $resolvedExePath }
            $trustedVendor = $trust -and
                ($Script:TrustedSigners -contains $trust.SignerSummary) -and
                ($trust.SignatureStatus -eq 'Valid') -and
                ($resolvedExePath -notmatch $userWritable)
            ...
            if ($trustedVendor -and $sev -eq 'HIGH') { $sev = 'MEDIUM' }
            elseif ($trustedVendor -and $sev -eq 'MEDIUM') { $sev = 'LOW' }

            # User-trusted task (your own tooling): keep the finding visible
            # for review, but cap it to LOW so it doesn't dominate the score.
            # This checks TaskName only, not the full path, on purpose -
            # matching by path would let an attacker plant a same-named task
            # in a different folder and inherit the trust.
            $isUserTrusted = $Script:TrustedTasks -contains $t.TaskName
            if ($isUserTrusted) {
                $reasons.Add('USER-TRUSTED task (listed in $Script:TrustedTasks)')
                $sev = 'LOW'
            }

            # Live process + parent chain, if this task's binary is currently running
            $chainStr = ''
            if ($resolvedExePath) {
                $proc = Get-CimInstance Win32_Process -Filter "ExecutablePath='$($resolvedExePath -replace "'","''")'" -ErrorAction SilentlyContinue | Select-Object -First 1
                if ($proc) {
                    $chain = Get-ProcessChain -ProcessId $proc.ProcessId
                    if ($chain.Count -gt 1) {
                        $names = $chain | ForEach-Object { $_.Name }
                        [array]::Reverse($names)
                        $chainStr = " | Chain: " + ($names -join ' -> ')
                    }
                }
            }

            $trustStr = ''
            if ($trust) { $trustStr = " | SHA256: $($trust.SHA256.Substring(0,16))... | Signer: $($trust.SignerSummary)" }

            Add-Finding -Category 'Task Scheduler' -Severity $sev -Type 'ScheduledTask' -Data $full -Description (
                "{0}  [{1}]  ({2})  {3}  | Author: {4} | RunAs: {5}/{6}{7}{8}" -f
                $full, ($reasons -join '; '), $t.State, $detail, $author, $runAs, $runLevel, $trustStr, $chainStr)
        }
    }
...
$failed = @($ModuleStatus | Where-Object Status -eq 'FAIL')
if ($failed) {
    Write-Line ("  {0} module(s) failed to complete - see report for details:" -f $failed.Count) Red
    $failed | ForEach-Object { Write-Line ("    {0}: {1}" -f $_.Module, $_.Error) Red }
}
